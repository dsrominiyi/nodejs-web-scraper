...
class OpenLinks extends HttpOperation {
  ...
  async scrape({ url, html }) {
    ...
    const hasOpenLinksOperation =
      this.operations.filter((child) => child.constructor.name === 'OpenLinks').length > 0; //Checks if the current page operation has any other page operations in it. If so, will force concurrency limitation.
