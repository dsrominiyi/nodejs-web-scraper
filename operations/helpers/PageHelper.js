...
class PageHelper {
   ...
   * @param {boolean} retryIfNoPageBody
   * @return {Promise<{data:[],address:string}>}
   */
  async processOneIteration(href, shouldPaginate, retryIfNoPageBody) {
    ...
    try {
      ...
      for (let i = 0; i < maxAttempts; i++) {
        response = await this.getPage(href);

        const shouldRetry =
          retryIfNoPageBody && (!response?.data || !response.data.includes('<body>'));
      ...
      var dataFromChildren = await this.Operation.scrapeChildren(this.Operation.operations, {
        html: response.data,
        url: response.url,
      });
